...
// Onglet Flux (« ce qui rentre, ce qui sort ») : regroupe le cash flow du mois,
// les charges fixes mensuelles, la détection d'abonnement, les dernières
// transactions et les récurrences à venir. Les listes renvoient aux écrans
// complets (Transactions / Récurrences) via les modales de App.jsx.
  ...
  return (
    <div style={{ minHeight: "100dvh", paddingBottom: "6rem" }}>
      ...
      >
        ...
        <h1 style={{ fontSize: 18, margin: 0, whiteSpace: "nowrap" }}>{t("nav_flux")}</h1>
        <div />
      </div>

      <div style={{ display: "flex", flexDirection: "column", gap: 14, padding: "0 1.25rem", maxWidth: 640, margin: "0 auto" }}>
        ...
        <WidgetCard icon="ti-list" accent="coral" title={t("flux_recent_title")} flush action={seeAll(() => onOpenTransactions?.())}>
          {recentTx.length === 0 ? (
        ...
        {/* Récurrences à venir */}
        <WidgetCard icon="ti-repeat" accent="pink" title={t("flux_upcoming_title")} flush action={seeAll(() => onOpenRecurring?.())}>
          {upcoming.length === 0 ? (
