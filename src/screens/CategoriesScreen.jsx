...
export default function CategoriesScreen() {
  ...
  const { categories, updateCategories, assets, incomeAccountLinks, setIncomeAccountLinks, customTags, updateCustomTags } = useFinance();
  const [expanded, setExpanded] = useState(null);
  const [showIncomeLinks, setShowIncomeLinks] = useState(false);
  const [showTags, setShowTags] = useState(false);
  const [newTagInput, setNewTagInput] = useState("");

  // Liste de tags gérée : la liste personnalisée du couple si elle existe,
  // sinon les presets par défaut (matérialisés dès la première modification).
  ...
  return (
    <div style={{ padding: "1.5rem 1.25rem 6rem" }}>
      ...
      {/* Gestion des tags : liste personnalisable (ajouter / réordonner /
          supprimer). Alimente les suggestions du champ de tags. */}
      <div
        style={{
          background: "var(--bg-card)",
          borderRadius: "var(--radius-lg)",
      ...
      >
        ...
        >
          <i className="ti ti-tags" style={{ fontSize: 18 }} aria-hidden="true" />
          <p style={{ fontSize: 14, fontWeight: 500, flex: 1 }}>{t("categories_tags_title")}</p>
          <i
            className={showTags ? "ti ti-chevron-down" : "ti ti-chevron-right"}
            style={{ fontSize: 14, color: "var(--ink-3)" }}
            aria-hidden="true"
          />
        </div>
